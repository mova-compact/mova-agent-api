...
fn contract_run_domain_bad_request(
    err: &crate::contract_run::ContractRunDomainError,
    message: &str,
) -> (StatusCode, Json<ErrorResponse>) {
    bad_request(&err.code, message, vec![err.message.clone()])
}
...
fn connector_unavailable(err: &ConnectorExecutionError) -> (StatusCode, Json<ErrorResponse>) {
    let status = if err.code == "connector_secret_missing" || err.code == "connector_secret_unavailable" {
        StatusCode::SERVICE_UNAVAILABLE
    } else if err.code == "connector_side_effect_denied"
        || err.code == "connector_denied"
        || err.code == "connector_ref_not_allowed"
        || err.code == "connector_ref_disabled"
        || err.code == "connector_operation_not_allowed"
        || err.code == "provider_not_supported"
        || err.code == "provider_operation_not_supported"
        || err.code == "connector_scope_denied"
    {
        StatusCode::FORBIDDEN
    } else {
...
fn transition_conflict(err: &crate::contract_run::ContractRunDomainError) -> (StatusCode, Json<ErrorResponse>) {
    (
        StatusCode::CONFLICT,
        Json(ErrorResponse {
            error: ApiError {
...
fn append_transition_failure_observation(
    state: &AppState,
...
async fn post_contract_run_step_execute(
    ...
    if payload.get("operation").is_some() || payload.get("side_effect_intent").is_some() {
        return (
            StatusCode::FORBIDDEN,
            Json(ErrorResponse {
                error: ApiError {
    ...
    let transition = match resolve_flow_transition(admitted_contract_ref, &executed_step_id, outcome) {
        ...
        Err(err) => {
            ...
            append_transition_failure_observation(
                &state,
                &mut run_state,
                &executed_step_id,
                outcome,
            ...
            return transition_conflict(&err).into_response();
        }
    };
