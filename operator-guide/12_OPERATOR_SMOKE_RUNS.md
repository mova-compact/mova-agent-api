...
- продолжение через `/decision` на live-контуре возвращает `404`:
  - `contract_run_not_found` или
  - `contract_not_found`.
...
- остановка работает;
- но продолжение не считается надёжно подтверждённым в текущем runtime.

## Smoke 7: Базовый `/actions/run`
...
- ожидалось создание run.

Проблема:

- фактически получен `403 connector_execution_failed` с `connector_denied`.
...
- этот путь сейчас ограничен настройками коннектора и не должен считаться “гарантированно рабочим” для ежедневного запуска.

## Вывод по текущему pass

- Основной операторский путь сейчас надёжнее вести через контрактные маршруты.
- Human gate как остановка подтверждён.
- Human gate как продолжение через `/decision` на live-контуре пока нестабилен.
